    ...
    SECTION("NVML depending methods succeed when NVML is available") {
        ...
        SECTION("GetAllClockFrequencies succeeds") {
            ...
            ALLOW_CALL(*nvml, DeviceGetClockInfo(_, NVML_CLOCK_VIDEO, _))
                ...
                .RETURN(NVML_SUCCESS);
            ALLOW_CALL(*nvml, DeviceGetMaxClockInfo(_, NVML_CLOCK_GRAPHICS, _))
                .LR_SIDE_EFFECT(*_3 = graphicsClock)
                .RETURN(NVML_SUCCESS);
            ALLOW_CALL(*nvml, DeviceGetMaxClockInfo(_, NVML_CLOCK_MEM, _))
                .LR_SIDE_EFFECT(*_3 = memoryClock)
                .RETURN(NVML_SUCCESS);
            ALLOW_CALL(*nvml, DeviceGetMaxClockInfo(_, NVML_CLOCK_VIDEO, _))
                .LR_SIDE_EFFECT(*_3 = videoClock)
                .RETURN(NVML_SUCCESS);
