...
NVAPI_FUNCTION NvAPI_GPU_GetGPCMask(NvPhysicalGpuHandle hPhysicalGpu, NvU32 *pSupport) {
...
NVAPI_FUNCTION NvAPI_GPU_GetTPCMaskOnGPC(NvPhysicalGpuHandle hPhysicalGpu, NvU32 Info, NvU32 *pSupport) {
    // Completely unknown. Experimental!
    constexpr auto n = __func__;
...
NVAPI_FUNCTION NvAPI_GPU_GetPstatesInfo(NvPhysicalGpuHandle hPhysicalGpu, NV_GPU_PERF_PSTATES_INFO *pPstatesInfo) {
    constexpr auto n = __func__;
    thread_local bool alreadyLoggedNoNvml = false;
    thread_local bool alreadyLoggedHandleInvalidated = false;
    thread_local bool alreadyLoggedOk = false;
...
NVAPI_FUNCTION NvAPI_GPU_ClientFanCoolersGetStatus(NvPhysicalGpuHandle hPhysicalGpu, NV_GPU_FANCOOLERSTATUS* pFanCoolerStatus) {
    constexpr auto n = __func__;
    thread_local bool alreadyLoggedNoNvml = false;
    thread_local bool alreadyLoggedHandleInvalidated = false;
    thread_local bool alreadyLoggedOk = false;
    ...
    for (unsigned int i = 0; i < numFans; i++) {
        ...
        if (resultSpeed == NVML_ERROR_NOT_SUPPORTED && resultRPM == NVML_ERROR_NOT_SUPPORTED)
            return NotSupported(n);
    }
